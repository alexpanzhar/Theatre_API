...
class AuthenticatedUserPerformanceTests(TestCase):
    ...
    def setUp(self):
        ...
        self.reservation = sample_reservation(self.user)
    ...
    def test_list_performances(self):
        ...
        # Retrieve performances and prepare the expected serialized data
        performances = Performance.objects.all()
        serializer = PerformanceListSerializer(performances, many=True)

        # Update serializer data to include tickets_available calculation
        for performance_data, performance in zip(
            serializer.data, performances
        ):
            tickets_available = (
                performance.theatre_hall.capacity - performance.tickets.count()
            )
            performance_data["tickets_available"] = tickets_available
